...
namespace BlockchainAssignment
{
    public partial class BlockchainApp : Form
    {
        ...
        private void NewBlock_Click(object sender, EventArgs e)
        {
            ...
            Block newBlock = new Block(blockchain.GetLastBlock(), transactions, publicKey.Text,4);
        ...
        /*GENERATE A NEW TRANSACTION BLOCK WITH A RANDOM DIFFICULTY*/
        private void Random_Difficulty_Click(object sender, EventArgs e)
        {
            // Retrieve pending transactions to be added to the newly generated Block
            List<Transaction> transactions = blockchain.GetPendingTransactions();
            Random randomizer = new Random(); //create the randomizer
            int difficulty = randomizer.Next(1, 6);//the difficulty gets randomly generated. it will be between 1 and 5
           
            // Create and append the new block - requires a reference to the previous block, a set of transactions and the miners public address (For the reward to be issued)
            Block newBlock = new Block(blockchain.GetLastBlock(), transactions, publicKey.Text,difficulty);
            blockchain.blocks.Add(newBlock);
