...
namespace BlockchainAssignment
{
    class Block
    {
        ...
        public static int difficulty; // An arbitrary number of 0's to proceed a hash value
        ...
        public Block(Block lastBlock, List<Transaction> transactions, String minerAddress, int difficulty_level)
        {
            timestamp = DateTime.Now;
        ...
        public String Mine()
        {
            ...
            timer.Start();
            String get_hash1, get_hash2;
            nonce = 0;
            Thread first_thread = new Thread(() => { get_hash1 = First_Miner_Thread(); });
            Thread second_thread = new Thread(() => { get_hash2 = Second_Miner_Thread(); });
            hash = Second_Miner_Thread();
            ...
            timer.Stop();
            TimeSpan time_span = timer.Elapsed;
            total_time = String.Format("{0:00}:{1:00}:{2:00}:{3:00}", time_span.Hours, time_span.Minutes, time_span.Seconds, time_span.Milliseconds);
            
            return hash;
        }

        private String First_Miner_Thread()
        {
        ...
        private String Second_Miner_Thread()
        {
        ...
        {
            return
                "Block Difficulty Level: " + difficulty
                + "\n[BLOCK START]"
                + "\nIndex: " + index
                + "\tTimestamp: " + timestamp
